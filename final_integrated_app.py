...
with col_main:
    ...
    if n_upload and e_upload:
        ...
        # --- 1. حساب الـ Metrics عشان نستخدمها في الفلاتر ---

        # بنحسب 3 Centrality Measures
        ...
        # بنحسب الـ Communities عشان نفلتر بيها
        ...
        # --- 2. بناء واجهة الفلاتر في القائمة الجانبية ---

        # هنرجع نكتب في العمود الأيسر تاني

        with col_left_menu:

            with st.expander("🎯 2. Advanced Filtering Options", expanded=True):

                st.write("**Filter by Centrality Ranges:**")

               

                # Sliders للـ 3 Centrality Measures
                ...
                # قائمة منسدلة (Multiselect) لاختيار المجتمعات
        ...
        for n, data in raw_Graph.nodes(data=True):
            ...
            # لو الـ Node بتحقق كل شروط الـ Sliders والـ Community، نحتفظ بيها
        ...
        with tab_viz:
            ...
            # --- إضافة قائمة لاختيار الـ Layout ---
            ...
            # حساب إحداثيات (أماكن) النقط بناءً على الخوارزمية المختارة

            with st.spinner("Calculating Layout positions..."):

                if layout_choice == "Force-Directed (Fruchterman-Reingold)":

                    pos = nx.spring_layout(final_Graph, seed=42) # spring_layout هو نفسه Fruchterman-Reingold
                    ...
                    pos = None # هنسيب Pyvis يتحرك براحته (بطيء)
            ...
            # إضافة النقط بالإحداثيات الثابتة عشان نلغي التحميل البطيء
            ...
            for n in final_Graph.nodes():
                ...
                hover_text = f"Node ID: {n} | Connections: {degrees.get(n)}"

               

                if pos is not None:

                    # بنضرب في 1000 عشان نوسع المسافات بين النقط في الرسمة

                    x_coord = pos[n][0] * 1000

                    y_coord = pos[n][1] * 1000

                    net.add_node(str(n), label=lbl, size=node_size, title=hover_text, color="#00ffcc", x=x_coord, y=y_coord, physics=False)

                else:

                    net.add_node(str(n), label=lbl, size=node_size, title=hover_text, color="#00ffcc")
            ...
            # قفل الـ Physics تماماً لو اخترنا Layout ثابت عشان الرسمة تفتح في ثانية
            ...
            if pos is not None:
                ...
                net.toggle_physics(True) # تشغيل الحركة في حالة الـ Live Physics بس
