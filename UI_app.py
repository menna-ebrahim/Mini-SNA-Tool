...
with col_main:
    ...
    if n_upload and e_upload:
        # قرأنا البيانات هنا كأرقام طبيعية جداً من غير أي تحويل لنصوص عشان الحسابات تطلع دقيقة
        ...
        tab_preview, tab_viz, tab_analysis, tab_comm = st.tabs([
            "📊 Data Preview", "🕸️ Interactive Viz", "🚀 PageRank & Betweeness Results", "🏘️ Community Detection Results"
        ])
        ...
        with tab_viz:
            ...
            # الحل الذكي: هنضيف النودز للرسمة كنصوص عشان Pyvis، بس الجراف الأصلي يفضل أرقام زي ما هو
            for n in final_Graph.nodes():
                net.add_node(str(n), size=degrees.get(n, 1) * 5, title=f"Node ID: {n} | Connections: {degrees.get(n)}", color="#00ffcc")
        ...
        with tab_analysis:
            if btn_pagerank:
                with st.spinner("Calculating PageRank..."):
                    ...
                    df_pr = pd.DataFrame(list(pr.items()), columns=["Node", "PageRank Score"]).sort_values(by="PageRank Score", ascending=False)
                    st.write("### PageRank Results")
            ...
            if btn_betweenness:
                with st.spinner("Calculating Betweenness Centrality..."):
                    ...
                    df_bc = pd.DataFrame(list(bc.items()), columns=["Node", "Betweenness Score"]).sort_values(by="Betweenness Score", ascending=False)
                    st.write("### Betweenness Results")
